...
<form  class="container" action='/admin/{{Request::segment(2)}}/update/{{$page->id}}' method="POST" enctype="multipart/form-data">
	@csrf
		...
		<input name="en_name" class="form-control"  id="comment" required value="{{$page->en_name}}">
		...
		<input name="ar_name" class="form-control"  id="comment" required value="{{$page->ar_name}}">
		
	</div>

				<div class="form-group">
		<label for="exampleInputEmail1">{{Request::segment(2)}} Main english Description</label>
		<textarea name="en_description" class="form-control summernote" rows="5" id="comment" required>{{$page->en_description}}</textarea>
		...
		<textarea name="ar_description" class="form-control summernote" rows="5" id="comment" required>{{$page->ar_description}}</textarea>
		
	</div>

			<div class="form-group">
		<label for="exampleInputEmail1">{{Request::segment(2)}} link</label>
		<input name="link" class="form-control"  id="comment" required value="{{$page->link}}">
		
	</div>


	<label for="exampleInputEmail1">{{Request::segment(2)}} Image</label>
	<input class="active" type="file" name="img_name" enctype="multipart/form-data" multiple>
	<br><br>
